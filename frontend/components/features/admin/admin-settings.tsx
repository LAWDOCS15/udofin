...
import { Save, Globe, Bell, Shield, Palette, ToggleLeft, ToggleRight, Loader2 } from "lucide-react"
...
export function AdminSettings() {
  ...
  const ToggleSwitch = ({ enabled, onToggle, label, desc }: { enabled: boolean; onToggle: () => void; label: string; desc: string }) => (
    <div className="flex items-center justify-between py-3 border-b border-border last:border-0">
      ...
      <button onClick={onToggle} className="relative transition-colors duration-200">
  ...
  return (
    <div className="p-6 lg:p-8 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={handleSave} disabled={isSaving} className="h-9 gap-2 rounded-xl bg-accent text-accent-foreground hover:bg-accent/90 text-xs font-semibold">
      ...
      <div className="grid gap-6 lg:grid-cols-4">
        <div className="rounded-2xl border border-border bg-card p-3 h-fit space-y-1">
          {sections.map((s) => (
            <button key={s.id} onClick={() => setSection(s.id)} className={cn("flex w-full items-center gap-2.5 rounded-xl px-3 py-2.5 text-sm font-medium transition-all", section === s.id ? "bg-accent/10 text-accent" : "text-muted-foreground hover:bg-secondary hover:text-foreground")}>
              {s.icon} {s.label}
            </button>
          ))}
        </div>
        ...
        <div className="lg:col-span-3 rounded-3xl border border-border bg-card p-6 shadow-sm">
          ...
          {section === "notifications" && (
            <div className="space-y-2 animate-in fade-in duration-200">
              <h2 className="text-base font-bold text-foreground mb-4">Notification Preferences</h2>
              <ToggleSwitch enabled={settings.emailNotifications as boolean} onToggle={() => {toggle("emailNotifications"); showToast("Preference updated", "success");}} label="Email Notifications" desc="Receive application updates via email" />
            </div>
          )}

          {section === "security" && (
            <div className="space-y-2 animate-in fade-in duration-200">
              <h2 className="text-base font-bold text-foreground mb-4">Security Settings</h2>
              <ToggleSwitch enabled={settings.auditLogging as boolean} onToggle={() => {toggle("auditLogging"); showToast("Preference updated", "success");}} label="Audit Logging" desc="Log all admin actions for security trail" />
            </div>
          )}

          {section === "appearance" && (
            <div className="space-y-2 animate-in fade-in duration-200">
              <h2 className="text-base font-bold text-foreground mb-4">Appearance</h2>
