...
export function NbfcPanel() {
  ...
  const fetchCounts = useCallback(async () => {
    try {
      const res = await axios.get("http://localhost:5000/api/nbfc/stats", { withCredentials: true })
      
      if (res.data && typeof res.data.pendingApplications !== 'undefined') {
        setPendingCount(res.data.pendingApplications)
      }
    } catch (err) {
  ...
  const SidebarContent = ({ isMobile = false }: { isMobile?: boolean }) => (
    <>
      <div className="flex items-center justify-between px-4 py-4 border-b border-border">
        {(!collapsed || isMobile) && <span className="text-sm font-bold text-foreground">NBFC Panel</span>}
        {!isMobile && (
          <button 
            type="button"
      ...
      <div className="border-t border-border px-2 py-3">
        <button type="button" className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-destructive hover:bg-destructive/10 transition-colors">
  ...
  return (
    <div className="flex min-h-screen bg-background pt-16">
      ...
      <main className={cn(
        ...
        <div className={cn("transition-all duration-200", transitioning ? "opacity-0 translate-y-1" : "opacity-100 translate-y-0")}>
          {renderContent()}
        </div>
      </main>
    </div>
