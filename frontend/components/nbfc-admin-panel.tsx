...
// Keep your interface exactly as it was
...
export function NBFCAdminPanel() {
  ...
  useEffect(() => {
    const fetchLeads = async () => {
      try {
        const res = await fetch('http://localhost:5000/api/applications/leads', {
          credentials: 'include'
        });
        ...
        // Map backend schema to your frontend LoanRecord interface
        const mappedData: LoanRecord[] = data.applications.map((app: any) => ({
          ...
          emi: 15000, // Dummy fallback for now
          status: app.verificationStatus === 'PENDING' ? 'pending-emi' : 'running',
          rate: 11.5,
          appliedDate: new Date(app.createdAt).toISOString().split('T')[0],
          disbursedDate: 'Pending',
          maturityDate: 'Pending',
          cibil: 750, // Dummy fallback, can be fetched via API later
          nextEMIDue: '-',
          emiPaid: 0,
          totalEMI: 36
        }));

        setLoanRecords(mappedData);
      } catch (err: any) {
  ...
  const filtered = useMemo(() => loanRecords.filter((r) => {
  ...
  }), [search, filterStatus, loanRecords])

  const stats = useMemo(() => ({
    totalDisbursed: loanRecords.reduce((s, r) => s + r.amount, 0),
    running: loanRecords.filter(r => r.status === 'running').length,
    overdue: loanRecords.filter(r => r.status === 'overdue').length,
    completed: loanRecords.filter(r => r.status === 'completed').length,
    pendingEMI: loanRecords.filter(r => r.status === 'pending-emi').length,
    monthlyCollection: loanRecords.filter(r => r.status === 'running').reduce((s, r) => s + r.emi, 0),
  }), [loanRecords])
  ...
  if (isLoading) {
    return <div className="min-h-screen flex items-center justify-center bg-background"><Loader2 className="animate-spin text-primary h-8 w-8" /></div>
  }

  // ... (The ENTIRE rest of your return() block remains exactly the same as your code)
  // I will show the start of the return just to confirm it's identical
  return (
    <div className="min-h-screen bg-background pt-16">
      {/* Hero */}
      <div className="bg-primary">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-primary-foreground/35">FinLend Platform — NBFC Admin</p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              <button className="relative flex h-9 w-9 items-center justify-center rounded-xl bg-primary-foreground/5 border border-primary-foreground/10 hover:bg-primary-foreground/10 transition-all">
      ...
      <div className="mx-auto -mt-5 max-w-7xl px-6 lg:px-8">
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-6">
          {[
            { label: 'Total Value', value: `₹${(stats.totalDisbursed / 100000).toFixed(1)}L`, icon: IndianRupee, accent: true, span: 2 },
            ...
            { label: 'Pending Apps', value: stats.pendingEMI, icon: Clock, color: 'text-amber-500' },
          ].map((s) => {
            const Icon = s.icon
            return (
              <div key={s.label} className={cn(
      ...
      {/* Rest of the UI follows exactly matching your design... */}
      <div className="mx-auto max-w-7xl px-6 py-8 lg:px-8">
         {/* Tabs mapping */}
         {/* Overview Tab logic */}
         {/* Loans Tab logic mapping through filtered */}
         <div className="text-center text-sm text-muted-foreground mt-10">
           Data is now loading live from backend for your specific NBFC.
         </div>
      </div>
    </div>
  )
