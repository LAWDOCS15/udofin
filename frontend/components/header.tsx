...
export function Header() {
  const { step, setStep, isAuthenticated, userRole, logout } = useOnboarding()
  ...
  const isHero = step === "landing"
  const isDark = !scrolled && isHero
  const isAdminView = step === "admin" || step === "nbfc-admin"
  ...
  const navItems = [
    { key: "landing", label: "Home", always: true },
    { key: "chatbot", label: "Apply", always: false, roles: ["user"] },
        ...
        <nav className="hidden items-center gap-1 md:flex">
          {navItems.map((item) => {
            ...
            return (
              <button
                key={item.key}
                onClick={() => !disabled && setStep(item.key as typeof step)}
                disabled={disabled}
        ...
        <div className="hidden items-center gap-2.5 md:flex">
          {isAuthenticated ? (
            <div className="flex items-center gap-3">
              ...
              <button onClick={logout} className={`flex items-center gap-1.5 text-[13px] font-medium transition-colors ${isDark ? "text-primary-foreground/40 hover:text-primary-foreground/70" : "text-muted-foreground hover:text-foreground"}`}>
            ...
            <>
              <button onClick={() => setStep("auth")} className={`text-[13px] font-medium transition-colors ${isDark ? "text-primary-foreground/50 hover:text-primary-foreground/80" : "text-muted-foreground hover:text-foreground"}`}>
                Sign in
              </button>
              <Button size="sm" className="h-9 gap-1.5 rounded-full bg-primary px-5 text-[13px] font-semibold text-primary-foreground hover:bg-primary/90 shadow-sm" onClick={() => setStep("auth")}>
        ...
        <div className="animate-fade-in border-t border-border/50 bg-card/95 backdrop-blur-xl md:hidden">
          <nav className="flex flex-col gap-1 px-6 py-4">
            {navItems.map((item) => {
              ...
              return (
                <button key={item.key} onClick={() => { if (!disabled) { setStep(item.key as typeof step); setMobileMenuOpen(false) } }} disabled={disabled}
                  className={`rounded-xl px-4 py-3 text-left text-sm font-medium transition-colors hover:bg-secondary ${step === item.key ? "bg-secondary text-foreground" : "text-muted-foreground"} ${disabled ? "cursor-not-allowed opacity-25" : ""}`}>
                  {item.label}
                </button>
              )
            })}
            <div className="mt-3 flex flex-col gap-2 border-t border-border pt-4">
              {isAuthenticated ? (
                <button onClick={() => { logout(); setMobileMenuOpen(false) }} className="w-full rounded-xl px-4 py-3 text-left text-sm font-medium text-muted-foreground hover:bg-secondary">
                  Sign out
                </button>
              ) : (
                <Button className="w-full rounded-xl bg-primary text-primary-foreground hover:bg-primary/90" onClick={() => { setStep("auth"); setMobileMenuOpen(false) }}>
                  Get started
                </Button>
              )}
            </div>
